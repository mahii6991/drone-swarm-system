...
fn stress_merkle_large_tree() {
    ...
    for i in 0..CAP/2 { // Fill up to capacity (leafs take half usually, depending on implementation details)
    ...
    match root {
        Ok(hash) => {
            println!("  ✅ Computed root for {} leaves in {:?}", data_refs.len(), duration);
...
fn bench_woa_convergence() {
    ...
    let min = Position { x: -100.0, y: -100.0, z: -100.0 };
    let max = Position { x: 100.0, y: 100.0, z: 100.0 };
    ...
    let cost_fn = |p: &Position| -> f32 {
        ...
        10.0 * 3.0 + (x*x - 10.0*cosf(2.0*pi*x)) + 
    ...
    println!("  Time per iteration: {:.2} ms", duration.as_millis() as f64 / 100.0);
}
...
fn stress_secure_aggregation_throughput() {
    ...
    println!("  ✅ Aggregated {} encrypted updates (100 params each) in {:?}", NUM_DRONES, duration);
